...
async def skip(cli, message: Message):
    ...
    if permission == "admin":
        ...
        user = await cli.get_chat_member(message.chat.id, message.from_user.id)
        if user.status != ChatMemberStatus.ADMINISTRATOR and user.status != ChatMemberStatus.OWNER:
            return await message.reply_text("❌ Oɴʟʏ ᴀᴅᴍɪɴs ᴄᴀɴ sᴋɪᴘ sᴏɴɢs ɪɴ ᴛʜɪs ɢʀᴏᴜᴘ.\n\nCᴏɴᴛᴀᴄᴛ ᴀɴ ᴀᴅᴍɪɴ ᴛᴏ sᴋɪᴘ ᴛʜᴇ ᴄᴜʀʀᴇɴᴛ ᴛʀᴀᴄᴋ.")
    elif permission == "member":
        # Admins and members can skip (but not anonymous)
        user = await cli.get_chat_member(message.chat.id, message.from_user.id)
        if user.status == ChatMemberStatus.RESTRICTED:
            return await message.reply_text("❌ Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ ᴘᴇʀᴍɪssɪᴏɴ ᴛᴏ sᴋɪᴘ sᴏɴɢs.")
    # "everyone" - no check needed, anyone can skip
