...
from config import API_KEY, API_URL, VIDEO_API_URL, NEXGENBOTS_API
from AnnieXMedia.logging import LOGGER

LOGGER = LOGGER(__name__)

# Use NextGen API if available, otherwise fall back to cookie-based download
USE_AUDIO_API = bool(API_URL and API_KEY)
USE_VIDEO_API = bool(VIDEO_API_URL and API_KEY)
USE_NEXGEN_API = bool(NEXGENBOTS_API and API_KEY)
...
def get_ytdlp_base_opts() -> Dict[str, object]:
    ...
    # Only use cookies if NextGen API is not available
    if not USE_NEXGEN_API:
...
async def download_from_nexgen_api(video_id: str, video: bool = False) -> Optional[str]:
    """Download video/audio from NextGen API instead of using cookies."""
    if not USE_NEXGEN_API:
        return None
    
    try:
        api_base = NEXGENBOTS_API.rstrip("/")
        endpoint = f"{api_base}/video/{video_id}" if video else f"{api_base}/audio/{video_id}"
            ...
            async with session.get(endpoint, params=params, timeout=aiohttp.ClientTimeout(total=60)) as resp:
                if resp.status == 200:
                    ...
                    if data.get("status") == "success":
                        download_url = data.get("result", {}).get("link") or data.get("url")
                        if download_url:
                            ext = "mp4" if video else "mp3"
                            out_path = f"{DOWNLOAD_DIR}/{video_id}.{ext}"
                            log_download_source(video_id, "NextGen API")
                            return await download_file(download_url, out_path)
    except Exception as e:
        LOGGER.debug(f"NextGen API download failed: {e}")
