...
skippermitdb = mongodb.skippermission  # New collection for skip permissions
...
skipmode = {}
mute = {}
skippermission = {}  # Cache for skip permissions: admin, member, everyone
...
# ── Skip Permission Functions ──────────────────────────────────────

async def get_skip_permission(chat_id: int) -> str:
    """Get skip permission setting for a chat (admin/member/everyone)"""
    mode = skippermission.get(chat_id)
    if not mode:
        user = await skippermitdb.find_one({"chat_id": chat_id})
        if not user:
            # Default to "admin" if not set
            skippermission[chat_id] = "admin"
            return "admin"
        skippermission[chat_id] = user.get("mode", "admin")
        return skippermission[chat_id]
    return mode


async def set_skip_permission(chat_id: int, mode: str):
    """Set skip permission for a chat (admin/member/everyone)"""
    if mode not in ["admin", "member", "everyone"]:
        raise ValueError("Invalid skip permission mode")
    
    skippermission[chat_id] = mode
    await skippermitdb.update_one(
