...
class Call:
    ...
    async def join_call(
        ...
        try:
            ...
            # Handle Telegram flood wait automatically
            wait_time = fw.value if hasattr(fw, 'value') else 17
            LOGGER(__name__).warning(f"Flood wait detected: {wait_time}s, waiting...")
            await asyncio.sleep(wait_time)
            # Retry once after waiting
            try:
                await assistant.play(chat_id, stream)
            except Exception as retry_err:
                raise AssistantErr(f"ᴜɴᴀʙʟᴇ ᴛᴏ ᴊᴏɪɴ ᴛʜᴇ ɢʀᴏᴜᴘ ᴄᴀʟʟ ᴀғᴛᴇʀ ᴡᴀɪᴛ. Rᴇᴀsᴏɴ: {retry_err}")
